...
//    public void user_clicks_on_set_change_billing_method() {
//        waitload();
////        settingsPage.changeBillingMethod();
//        settingsPage.setBillingMethod();
//        waitload();
//    }

    @When("^user clicks on set/change billing method$")
    public void user_clicks_on_set_change_billing_method() {
        waitload();
        if (settingsPage.savedBilingMethodCheck().isEmpty()){
            settingsPage.setBillingMethodBtnClick();
        }
        else if (!settingsPage.savedBilingMethodCheck().isEmpty()){
            settingsPage.changeBillingMethodBtnClick();
        }
        else {
            System.out.println("Billing Method error.");
        }
    ...
    public void user_selects_debit_or_credit_card_from_method_dropdown() throws InterruptedException {
        ...
        settingsPage.card();
        waitload();
        settingsPage.savedCardListCheck();
        Thread.sleep(1000);
        waitload();
    }
    ...
    public void user_selects_an_added_card_and_clicks_save_button() throws InterruptedException {
        driver.manage().timeouts().implicitlyWait(20,TimeUnit.SECONDS);
        settingsPage.selectAddedCard();
        Thread.sleep(1000);

    }

    @Then("saved card number should show in settings page")
    public void saved_card_number_should_show_in_settings_page() throws Exception {
        driver.navigate().refresh();
        user_clicks_on_settings_button();
        driver.manage().timeouts().implicitlyWait(20,TimeUnit.SECONDS);
        Assert.assertTrue("Saved card doesn't appear in Settings page",settingsPage.verifySavedCard());
    ...
    public void user_should_see_message() throws InterruptedException {
        waitload();
        Thread.sleep(1500);
    ...
    public void newly_added_card_should_appear_in_the_list() throws InterruptedException {
//        driver.manage().timeouts().implicitlyWait(20, TimeUnit.SECONDS);
        waitload();
        Thread.sleep(5000);
        settingsPage.methodDropdown();
        ...
        settingsPage.afterCardCount();
        Thread.sleep(1000);
        Assert.assertTrue("Add card unsuccessful!!!",  settingsPage.addedCardCheck());
        waitload();
    }
